...
/// Adapter errors that can be recovered only after the user completes a
/// provider-owned authorization flow again. AppServices uses this small
/// protocol instead of depending on a concrete adapter module.
public protocol OnlineSourceAuthorizationRecoverableError: Error {
    var requiresUserAuthorization: Bool { get }
}
