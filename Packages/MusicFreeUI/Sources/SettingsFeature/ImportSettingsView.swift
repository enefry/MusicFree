...
struct ImportSettingsView: View {
    ...
    var body: some View {
        Section(L("导入与资料库")) {
            Picker(L("重复内容"), selection: duplicatePolicyBinding) {
    ...
    private func policyDescription(_ policy: DuplicateImportPolicy) -> String {
        switch policy {
        case .skipExisting:
            return L("检测到相同内容时跳过导入，避免资料库出现重复歌曲。")
        case .replaceExisting:
            return L("保存替换意图；当前内容寻址资料库仍使用安全的跳过行为，")
                + L("替换事务接入后生效。")
        case .keepBoth:
            return L("保存保留副本意图；当前内容寻址资料库仍使用安全的跳过行为，")
                + L("重复副本事务接入后生效。")
        }
    }
}
